...
exports.createMenu = asyncHandler(async (req, res) => {
  ...
  const nameUnique = await FooterMenu.find({
...
exports.getSlugMenu = asyncHandler(async (req, res) => {
  ...
  firstParent = await FooterMenu.findOne({ slug: mainParent })
    .where("parentId")
    .equals(undefined);
  menu = firstParent;
  childeMenus = await FooterMenu.find({})
    .where("parentId")
    .equals(firstParent._id);

  if (slugArray.length === 2) {
    leadParent = firstParent;
    menu = await FooterMenu.findOne({ slug: slugArray[slugArray.length - 1] })
      .where("parentId")
      .equals(leadParent._id);
    childeMenus = await FooterMenu.find({}).where("parentId").equals(menu._id);
  }

  if (slugArray.length === 3) {
    leadParent = await FooterMenu.findOne({
  ...
  if (childeMenus.length <= 0) {
    sameParentMenus = await FooterMenu.find({})
      .where("parentId")
      .equals(leadParent._id);
  }
